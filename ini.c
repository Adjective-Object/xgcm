...
#define BLOCK_CHUNK_SIZE 12
static char *read_block_section(FILE *file) {
    char * buf = malloc(sizeof(char) * BLOCK_CHUNK_SIZE);
    size_t len = BLOCK_CHUNK_SIZE;
    size_t index = 2;
    fread(buf, sizeof(char), 2, file);
    
    while (memcmp(buf+index-2, "\n[", 2) != 0) {
        fread(buf+index, sizeof(char), 1, file);
        index ++;
        if (len == index) {
            len += BLOCK_CHUNK_SIZE;
